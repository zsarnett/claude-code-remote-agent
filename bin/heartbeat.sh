...
HEARTBEAT_FILE="${HEARTBEAT_FILE:-$HOME/HEARTBEAT.md}"
NOTIFY="$HOME/.claude/bin/discord-notify.sh"
HUB_CHANNEL="${DISCORD_HUB_CHANNEL_ID:-YOUR_HUB_CHANNEL_ID}"
...
- Run real commands to check (git status, df, tmux list-sessions, docker ps, etc.)" --dangerously-skip-permissions 2>/dev/null)
